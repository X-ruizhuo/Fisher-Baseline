...
from reid.utils.fisher import fisher_consolidation_loss
import copy
...
class Trainer(object):
    ...
    def train(self, epoch, data_loader_train,  optimizer, training_phase,
              ...
              old_params=None, old_fisher=None,
              ):
        ...
        for i in range(train_iters):
            ...
            # Fisher 参数固化属于参数空间约束，与 Bi-C2R 的特征蒸馏保持解耦。
            # 仅在显式开启且传入历史状态时生效，关闭开关即可复现原始 baseline。
            if (
                getattr(self.args, 'use_fisher', False)
                and old_fisher is not None
                and old_params is not None
            ):
                loss_fim = fisher_consolidation_loss(
                    self.model,
                    old_params,
                    old_fisher,
                    old_classifier_rows=add_num,
                )
                loss = loss + self.args.lambda_fim * loss_fim
                losses_fim.update(loss_fim.item())
            optimizer.zero_grad()
            loss.backward()
            ...
            if self.writer != None :
                ...
                self.writer.add_scalar(tag="loss/Loss_dc_{}".format(training_phase), scalar_value=losses_dc.val,
                          global_step=epoch * train_iters + i)
                self.writer.add_scalar(tag="loss/Loss_fim_{}".format(training_phase), scalar_value=losses_fim.val,
                          global_step=epoch * train_iters + i)
